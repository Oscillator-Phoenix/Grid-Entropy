...
int main() {
    ...
    while (getNextSample(stream, &width, &height, &sample) != 0) {
        ...
        // // Use OpemMP version below:
        // result = (float*)malloc(sizeof(float)*width*height);
        // st = getTime();
        // openmp_entropy(width, height, sample, result);
        // et = getTime();
        // free(result);

        // Invoke the callback
        st = getTime();
        cudaCallback_opt(width, height, sample, &result);
        // cudaCallback_base(width, height, sample, &result);
        et = getTime();
